 ...
 *
 */
public class FindFaceForCameraTask extends Task {
	...
	public FindFaceForCameraTask(Context context) {
		super(context);
		isFinding = false;
	...
	private List<ConnectedImage> getFaces(BufferedImage img) {
		...
		if(!isLastSuccess) {
			...
			hasSkinColor = !hasSkinColor;
		}
		if(lastScale != 1) {
			img = ImageUtil.imageScale(img, lastScale, lastScale);
		...
		if(hasSkinColor) {
			...
			result.addAll(adaboost.getFacesForRealTime());
		}
		if(result.size() > 0) {
			isLastSuccess = true;
			if(lastScale != 1) {
